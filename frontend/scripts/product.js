...
function createCategorySection(data, title) {
  const main = document.querySelector("main");
...
async function getCategoryProducts(category, title) {
  try {
    ...
    const result = await response.json();
    const data = result.data;
    createCategorySection(data, title);
  } catch (error) {
    console.log(error);
  }
}
...
getSuggestProducts();
getCategoryProducts("laptop", "دسته بندی لپ‌تاپ");
getCategoryProducts("headset", "دسته بندی هدست");
